   ...
   void Jump() 
{
    ...
    if (Input.GetButtonDown("Jump") && Input.GetAxisRaw("Vertical") >= 0) 
    {
        ...
        if (onWall() && !currentlyGrounded) {
            ...
            rb.linearVelocity = new Vector2((transform.localScale.x > 0 ? -1 : 1) * wallPushPower, wallJumpPower);
        }
    }
    ...
    anim.SetBool("Jumping", (!currentlyGrounded && !onWall() && rb.linearVelocity.y > 0));
    anim.SetBool("Falling", (!currentlyGrounded && !onWall() && rb.linearVelocity.y <= 0));
}
    ...
    void UpdateGravity() {
        ...
        if (!Grounded() && !onWall() && rb.linearVelocity.y >= 0) 
        {
